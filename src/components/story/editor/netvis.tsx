...
function NetVis(props: IVisContentProps) {
    const {visTypeList, network, options, storyId, editorId, uniqueId} = props
    ...
    const update = async () => {
        for (let index in visTypeList) {
            ...
            //const containerIds = `visSvg${index}`
            //let containerId = `visSvg${index}story${storyId}ed${editorId}`
            let containerId = `visSvg${index}el${uniqueId}`
    ...
    const resizeChange = () => {
        ...
        visTypeList.map((visType, idx) => {
            const container = document.getElementById(`visSvg${idx}el${uniqueId}`)
            if (!container) {
    ...
    return (
        loading ?
            ...
            (<div style={{width: '100%', height: '100%', display: 'flex'}}>
                {visTypeList.map((visType, idx) => {
                    return (
                        <div key={idx} id={`visSvg${idx}el${uniqueId}`}
